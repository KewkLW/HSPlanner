...
export { newId, StorageCapacityError, StorageWriteError } from '../storage'
...
function isStashEntry(v: unknown): v is StashEntry {
  ...
  return (
    ...
    o.item !== null &&
    typeof (o.item as Record<string, unknown>).baseId === 'string'
  )
}

export function sanitizeStash(raw: unknown): StashEntry[] {
  if (!Array.isArray(raw)) return []
  return raw.filter(isStashEntry).slice(0, MAX_STASH_ENTRIES)
}
...
export function setBuildStash(
  ...
  const build = library.builds.find((b) => b.id === buildId)
  if (!build) return null
  build.stash = sanitizeStash(stash)
  build.updatedAt = new Date().toISOString()
  writeLibrary(library)
  return build
}
...
export function nextDuplicateName(base: string, taken: string[]): string {
