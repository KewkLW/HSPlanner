...
import { EhpSummary } from '../components/EhpSummary'
import { EhpRows } from '../components/EhpRows'
...
export default function CharacterView() {
  ...
  const avgHitDpsMax = performance?.avgHitDpsMax
  const rate =
  ...
  const isSpellSkill = !!mainSkills[0]?.tags?.includes('Spell')
  const critChanceKey = isSpellSkill ? 'spell_crit_chance' : 'crit_chance'
  const critDamageKey = isSpellSkill ? 'spell_crit_damage' : 'crit_damage'
